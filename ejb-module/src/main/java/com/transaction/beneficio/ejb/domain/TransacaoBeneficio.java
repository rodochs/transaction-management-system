...
public class TransacaoBeneficio {
    ...
    public TransacaoBeneficio(ContaBeneficio contaOrigem,
        ...
        if (tipo == TipoTransacao.TRANSFERENCIA && (contaOrigem == null || contaDestino == null)) {
            throw new IllegalArgumentException("Transfer must have both origin and destination accounts");
        }
        if ((tipo == TipoTransacao.CREDITO || tipo == TipoTransacao.DEBITO) && contaOrigem == null) {
            throw new IllegalArgumentException("Single-account operation must have an origin account");
        }
